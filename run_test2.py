...
def run_testing(param):
    def get_test_op(model, batch_dict, ent_embedding, id_entity_map):
        ...
        return loss, dec_op, mem_entries_sorted, gold_entity_ids, batch_orig_response, mem_tuples_sorted

    def perform_test(model, batch_dict, batch_target_word_ids, batch_text_targets, vocab, ent_embedding, id_entity_map):
        # print 'reading model from  modelfile'
        # saver.restore(sess, model_file)
        # test_data = pkl.load(open(param['test_data_file']),'rb')
        # print "Test dialogues loaded"
        ...
        # write_to_file(test_predicted_sentence, test_text_targets)
        # print ('average test loss is =%.6f' %(float(test_loss)/float(len(test_data))))
        batch_test_loss, test_op, prob_mem_entries, gold_entity_ids, gold_orig_response, prob_mem_tuples = get_test_op(model, batch_dict, ent_embedding, id_entity_map)
        ...
        print_pred_true_op(batch_predicted_sentence, prob_predicted_words, prob_true_words, batch_text_targets, batch_test_loss, prob_mem_entries, gold_entity_ids, gold_orig_response, prob_mem_tuples)
        sys.stdout.flush()
        return sum_batch_loss

 #    def get_train_loss(model, batch_dict, feed_prev, step, ent_embedding):
 #        train_batch_enc_w2v, train_batch_enc_kb, batch_target, batch_orig_target, batch_response, batch_orig_response, batch_text_weight, batch_mem_weight, batch_decoder_input, batch_sources, batch_rel, batch_key_target = get_batch_data(param['max_len'], param['max_utter'], param['memory_size'], param['gold_target_size'], param['batch_size'], batch_dict)
 #        feed_dict = feeding_dict(model, train_batch_enc_w2v, train_batch_enc_kb, batch_orig_target,  batch_target, batch_response, batch_text_weight, batch_mem_weight, batch_decoder_input, batch_sources, batch_rel, batch_key_target, feed_prev, ent_embedding)
 #        loss, dec_op, _ = sess.run([losses, logits, train_op], feed_dict=feed_dict)
 #        return loss, dec_op

 #    def get_valid_loss(model, batch_dict, ent_embedding, id_entity_map):
 #        val_batch_enc_w2v, val_batch_enc_kb, batch_target, batch_orig_target, batch_response, batch_orig_response, batch_text_weight, batch_mem_weight, batch_decoder_input, batch_sources, batch_rel, batch_key_target = get_batch_data(param['max_len'], param['max_utter'], param['memory_size'], param['gold_target_size'], param['batch_size'], batch_dict)
 #        feed_dict = feeding_dict(model, val_batch_enc_w2v, val_batch_enc_kb, batch_orig_target, batch_target, batch_response, batch_text_weight, batch_mem_weight, batch_decoder_input, batch_sources, batch_rel, batch_key_target, True, ent_embedding)
 #        loss, dec_op, prob_mem_entries = sess.run([losses, logits, prob_mem], feed_dict)
 #        prob_mem_entries = np.array(prob_mem_entries)
 # 	mem_entries_sorted = np.fliplr(np.argsort(prob_mem_entries,axis=1))
	# #batch_inverse_mapped_target is of dimension batch_size * mem_size + 1, each entry being the global index
	# mem_entries_sorted = [[batch_key_target[j][i] for j in mem_entries_sorted[i]] for i in range(batch_key_target.shape[1])]
	...
	# gold_entity_ids = np.transpose(batch_orig_target, (1,0))
	# gold_entity_ids = [[id_entity_map[gold_entity_ids[i][j]] for j in range(gold_entity_ids.shape[1])] for i in range(gold_entity_ids.shape[0])]
 #        return loss, dec_op, mem_entries_sorted, gold_entity_ids, batch_orig_response

    def get_sum_batch_loss(batch_loss):
        return np.sum(np.asarray(batch_loss))

    # def perform_training(model, batch_dict, feed_prev, step, ent_embedding):
    #     batch_train_loss, dec_op = get_train_loss(model, batch_dict, feed_prev, step, ent_embedding)
    #     sum_batch_loss = get_sum_batch_loss(batch_train_loss)
    #     return sum_batch_loss

    # def perform_evaluation(model, batch_dict, batch_target_word_ids, batch_text_targets, epoch, step, vocab, ent_embedding, id_entity_map):
    #     batch_valid_loss, valid_op, prob_mem_entries, gold_entity_ids, gold_orig_response = get_valid_loss(model, batch_dict, ent_embedding, id_entity_map)
    ...
    #     print_pred_true_op(batch_predicted_sentence, prob_predicted_words, prob_true_words, batch_text_targets, step, epoch, batch_valid_loss, prob_mem_entries, gold_entity_ids, gold_orig_response)
    #     return sum_batch_loss

    # def evaluate(model, epoch, step, valid_data, valid_text_targets, vocab, ent_embedding, id_entity_map):
    ...
    #         sum_batch_loss = perform_evaluation(model, batch_dict, batch_target_word_ids, batch_target_sentences, epoch, step, vocab, ent_embedding, id_entity_map)
    #         valid_loss = valid_loss + sum_batch_loss
    #     return float(valid_loss)/float(len(valid_data))

    def print_pred_true_op(pred_op, prob_pred, prob_true, true_op, batch_valid_loss, prob_memory_entities, gold_entity_ids, gold_orig_response, prob_mem_tuples):
	test_result_dir = param['test_output_dir']
        ...
        top20_mem_ent_file = os.path.join(test_result_dir, 'top20_mem_ent.txt')
        top20_kb_ent_file = os.path.join(test_result_dir, 'top20_kb_ent.txt')
        ...
        f1 = open(true_sent_file,'a')
        f2 = open(pred_sent_file,'a')
        f3 = open(top20_mem_ent_file,'a')
        f4 = open(top20_kb_ent_file,'a')
        f5 = open(gold_ent_file,'a')
        f6 = open(gold_resp_file,'a')
        f7 = open(batch_loss_file,'a')
        f8 = open(top20_mem_tuple_file,'a')

        for i in range(0,len(true_op)):
            # print "true sentence in  is:"
            sys.stdout.flush()
            f1.write('%s\n' % true_op[i])
            # print "predicted sentence in  is:"
            sys.stdout.flush()
            kb_name_list = [unidecode.unidecode(wikidata_id_name_map[id]) for id in prob_memory_entities[i] if id in wikidata_id_name_map if wikidata_id_name_map[id] not in ['<pad_kb>','<nkb>','<unk>']]
            kb_name_list_unique = list(OrderedDict.fromkeys(kb_name_list))[:20]

            pred_op_new = pred_op[i].split(' ')
            k = 0
	    max_k = 2
	    ...
	    pred_op_new = re.sub(' +',' ',' '.join(pred_op_new)).strip()
    	    ...
    	    # print "top-5 memory entries in  is:"
       	    f3.write('%s\n' % ', '.join([unidecode.unidecode(mem_entry) for mem_entry in prob_memory_entities[i] if mem_entry not in ['<pad_kb>','<nkb>','<unk>']][:20]))
    	    # print "top-5 KB entities in  is:"
    	    f4.write('%s\n' % ', '.join([unidecode.unidecode(wikidata_id_name_map[id]) for id in prob_memory_entities[i] if id in wikidata_id_name_map if wikidata_id_name_map[id] not in ['<pad_kb>','<nkb>','<unk>']][:20]))
    ...
    with tf.Graph().as_default():
        ...
        losses, logits, prob_mem = model.inference()
        ...
        for i in range(n_batches):
            ...
            batch_target_sentences = map_id_to_word(batch_target_word_ids, vocab)
            sum_batch_loss = perform_test(model, batch_dict, batch_target_word_ids, batch_target_sentences, vocab, ent_embed, id_entity_map)
